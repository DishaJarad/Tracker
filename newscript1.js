...
// Initialize destinationCoords as an empty array (placeholder)
...
function getCoordinatesFromAddress(address) {
  ...
  function updateSourcePosition(currentCoords) {
   //const map = L.map("map"); // Default map center, can be changed
   // L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png").addTo(map);
...
function updateDestination(coords) {
  ...
  console.log("We are in  updatedestination function");
  if (window.destinationMarker) {
    map.removeLayer(window.destinationMarker);
  }
console.log("Function 1 o updatefunction is running");

if (!destinationIcon) { console.error('Destination icon is not initialized'); return; }
...
// Example usage: Call the function with a place name
 // Replace with any place name
...
function setupRouting(startCoords, endCoords) {
  console.log("we are in setup routing fun");
  let routeControl = null;
  if (routeControl) {
    map.removeControl(routeControl); // Remove the previous route
  }
